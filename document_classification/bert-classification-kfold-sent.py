...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, df):
        ...
        self.tagged_sent = [tokenizer(text, padding='max_length', max_length = 512, truncation=True, return_tensors="pt") for text in df['tagged_sent']]
...
class BertClassifier(nn.Module):
    ...
    def forward(self, input_id, mask, tagged_mask):
        ...
        _, tagged_pooled_output = self.bert(input_ids= input_id, attention_mask=tagged_mask,return_dict=False)
        concat_pooled_output = torch.cat([pooled_output, tagged_pooled_output], dim=1)  
...
df = data[['abstract', 'task', 'tagged_sent']]
